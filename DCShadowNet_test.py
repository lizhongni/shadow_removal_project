...
class DCShadowNet(object) :
    ...
    def __init__(self, args):        
        ...
        print("# samplepath : ", self.samplepath)
    ...
    def build_model(self):
        ...
        self.testA = ImageFolder(os.path.join(self.samplepath), self.test_transform)
    ...
    def test(self):
        ...
        if not len(model_list) == 0:
            ...
            for i in range(-1,0,1):
                ...
                path_fakeB=os.path.join(self.result_dir, self.dataset,'model','output')
                print('output saved in:', path_fakeB)
                if not os.path.exists(path_fakeB):
                    os.makedirs(path_fakeB)

                path_realAfakeB=os.path.join(self.result_dir, self.dataset,'model','input_output')
                print('input_output saved in:', path_realAfakeB)
                if not os.path.exists(path_realAfakeB):
                    os.makedirs(path_realAfakeB)

                self.test_list = [os.path.splitext(f) for f in os.listdir(os.path.join(self.samplepath)) if any(f.endswith(suffix) for suffix in IMG_EXTENSIONS)]
                for n, in_name in enumerate(self.test_list):
                    print('predicting: %d / %d' % (n + 1, len(self.test_list)))
                    img_name = in_name[0]
                    im_suf   = in_name[-1]
                    img = Image.open(os.path.join(self.samplepath, img_name + im_suf)).convert('RGB')
